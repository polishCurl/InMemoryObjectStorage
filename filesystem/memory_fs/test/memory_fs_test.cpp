...
TEST(MemoryFsPut, Success) {
  ...
  ASSERT_EQ(Result::kSuccess, ms.add("test_file.txt", file));
  File another_file{1, static_cast<char>(0x11)};
  EXPECT_EQ(Result::kSuccess, ms.add("path.test_file.txt", another_file));
}

TEST(MemoryFsPut, AlreadyExists) {
  MemoryFs ms;
  File file{10, static_cast<char>(0xed)};
  ASSERT_EQ(Result::kSuccess, ms.add("a.out", file));
  File another_file{1, static_cast<char>(0x11)};
  EXPECT_EQ(Result::kAlreadyExists, ms.add("a.out", another_file));
}
...
TEST(MemoryFsGet, FileNotFound) {
  ...
  File file{3, 'z'};
  ASSERT_EQ(Result::kSuccess, ms.add("some/path/to/file_other", file));
  ASSERT_EQ(Result::kFileNotFound, ms.get("some/path/to/file").first);
}
...
TEST(MemoryFsGet, Success) {
  MemoryFs ms;
  File file{3, 'z'};
  ASSERT_EQ(Result::kSuccess, ms.add("/tmp/temp.txt", file));
...
TEST(MemoryFsList, Success) {
  ...
  ASSERT_EQ(Result::kSuccess, ms.add("b.hpp", file));
  ASSERT_EQ(FileList{"b.hpp"}, ms.list());

  ASSERT_EQ(Result::kSuccess, ms.add("a.hpp", file));
